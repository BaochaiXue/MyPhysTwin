...
from torchvision.ops import (
    box_convert,
)  # Converts bounding boxes between coordinate conventions.
from sam2.build_sam import build_sam2  # Factory for SAM2 image model construction.
from sam2.sam2_image_predictor import (
    SAM2ImagePredictor,
)  # High-level API exposing SAM2 image segmentation.
from groundingdino.util.inference import (
    load_model,
    load_image,
    predict,
)  # GroundingDINO helpers for detection-style prompting.
from argparse import (
    ArgumentParser,
)  # Parses command-line arguments controlling inputs and prompts.
...
)  # Destination path for the generated RGBA mask composite.
parser.add_argument(
    "--TEXT_PROMPT", type=str
)  # Natural-language prompt used to locate the object within the image.
...
TEXT_PROMPT = (
    args.TEXT_PROMPT
)  # Text description guiding GroundingDINO to the correct object.

SAM2_CHECKPOINT = "./data_process/groundedSAM_checkpoints/sam2.1_hiera_large.pt"  # Local weight file for SAM2 (image mode).
SAM2_MODEL_CONFIG = (
    "configs/sam2.1/sam2.1_hiera_l.yaml"  # Configuration describing model architecture.
)
GROUNDING_DINO_CONFIG = "./data_process/groundedSAM_checkpoints/GroundingDINO_SwinT_OGC.py"  # Config file referencing the Swin-T version of GroundingDINO.
GROUNDING_DINO_CHECKPOINT = "./data_process/groundedSAM_checkpoints/groundingdino_swint_ogc.pth"  # Checkpoint file containing pretrained weights for object grounding.
BOX_THRESHOLD = (
    0.35  # Minimum box score from GroundingDINO to treat detections as valid prompts.
)
TEXT_THRESHOLD = 0.25  # Minimum text score ensuring the textual match is confident.
DEVICE = (
    "cuda" if torch.cuda.is_available() else "cpu"
)  # Select GPU when available for faster inference.
...
)  # Load SAM2 weights on the chosen device.
sam2_predictor = SAM2ImagePredictor(
    sam2_model
)  # Wrap the raw model in a predictor API exposing `.predict`.
...
image_source, image = load_image(
    img_path
)  # Read the image in both numpy and tensor-friendly formats expected by GroundingDINO.
...
boxes = boxes * torch.Tensor(
    [w, h, w, h]
)  # Convert relative coordinates (cx, cy, w, h) into pixel space.
input_boxes = box_convert(
    boxes=boxes, in_fmt="cxcywh", out_fmt="xyxy"
).numpy()  # Translate to (x_min, y_min, x_max, y_max) format for SAM2.
...
).__enter__()  # Enable mixed-precision inference to reduce GPU memory footprint.

if torch.cuda.get_device_properties(0).major >= 8:
    # turn on tfloat32 for Ampere GPUs (https://pytorch.org/docs/stable/notes/cuda.html#tensorfloat-32-tf32-on-ampere-devices)
    torch.backends.cuda.matmul.allow_tf32 = (
        True  # Allow TensorFloat-32 for faster matmuls on supported hardware.
    )
    torch.backends.cudnn.allow_tf32 = (
        True  # Enable TF32 inside cuDNN convolutions for additional speed.
    )
...
if masks.ndim == 4:
    masks = masks.squeeze(
        1
    )  # Remove singleton dimension when SAM2 returns masks with explicit channel axis.


confidences = (
    confidences.numpy().tolist()
)  # Convert confidences back to vanilla Python lists for downstream prints.
...
}  # Map integer indices to readable class names (debugging aid).

print(
    f"Detected {len(masks)} objects"
)  # Show how many masks SAM2 produced with the configured prompt.

raw_img = cv2.imread(
    img_path
)  # Reload the image via OpenCV to extract raw pixel values for compositing.
mask_img = (masks[0] * 255).astype(
    np.uint8
)  # Convert the first predicted mask into an 8-bit grayscale image.

ref_img = np.zeros(
    (h, w, 4), dtype=np.uint8
)  # Prepare an RGBA canvas initialised to transparent.
mask_bool = mask_img > 0  # Interpret non-zero mask pixels as foreground.
ref_img[mask_bool, :3] = raw_img[
    mask_bool
]  # Copy RGB pixels from the original image wherever the mask is active.
ref_img[:, :, 3] = (
    mask_bool.astype(np.uint8) * 255
)  # Set alpha channel: opaque for foreground, transparent otherwise.
cv2.imwrite(
    output_path, ref_img
)  # Write the composited RGBA image for TRELLIS consumption.
